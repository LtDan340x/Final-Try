const AppState = (() => {
  const STORAGE_KEY = 'htown-hitters-race-state-v22-supabase';
  const SYNC_MODE_KEY = 'htown-hitters-sync-mode-v22-supabase';
  const bc = 'BroadcastChannel' in window ? new BroadcastChannel('htown_hitters_race_sync_v22_supabase') : null;
...
async function saveSupabaseState(state) {
  ...
  async function setState(nextState) {
    ...
    const mode = await resolveTransportMode();
    if (mode.actual === 'online') {
      await saveSupabaseState(normalized);
